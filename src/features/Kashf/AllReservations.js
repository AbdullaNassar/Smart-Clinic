...
function AllReservations(){
    const{handlePatientID, handleBookingID, handleReservationID}=usePatient();
    ...
    const { data:patients, isLoading:loading, isError } = useQuery(['data'], async () => {
    ...
    const { data:booking, isLoading:load, isError:errorr } = useQuery(['booking'], async () => {
    ...
    return(
        <div>
            <table className={classes.customers}>
                ...
                {!isLoading&&!loading &&!load&&booking!==undefined &&patients!==undefined&&reservations.map((item,idx)=>
                <tr>
