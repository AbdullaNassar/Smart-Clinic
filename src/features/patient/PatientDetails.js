...
function PatientDetails(){
    ...
    console.log(data);
    const {isLoading:loadingBooking, data:bookings, errorBooking}= useQuery({
        queryKey:['booking'],
        queryFn: getBooking,
    })
    ...
    return(
        <div className={classes.all}>
            ...
            <div className={classes.details}>
                ...
                {!loadingBooking&&<div className={classes.allBookings}>
                    {filteredList!==undefined&& filteredList.map((item,idx)=><div className={classes.card}>
                        <div>{idx+1}</div>
                        <div>
                        ...
                        <button onClick={()=>{
                            navigate(`/ReservationDetails?patID=${id}&bokID=${item.id}`)
                        }}>فتح الكشف</button>
                    </div>)}
                </div>}
            </div>
